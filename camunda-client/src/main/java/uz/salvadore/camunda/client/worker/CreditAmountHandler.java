...
@ExternalTaskSubscription(
  topicName = CamundaTopics.CREDIT_AMOUNT,
  lockDuration = 60000,
...
public class CreditAmountHandler implements ExternalTaskHandler {
  ...
  public void execute(ExternalTask externalTask, ExternalTaskService externalTaskService) {
    try {
      ...
      paymentService.creditAmountToProvider(appId, "PAYME", new BigDecimal("100.00"));
      service.updateState(appId, State.CREDIT_AMOUNT);
      externalTaskService.complete(externalTask, Map.of(Constants.TASK_STATE, State.CREDIT_AMOUNT));
    } catch (Exception ex) {
      log.error("CreditAmountHandler: {}", ExceptionUtils.getMessage(ex));
      if (ex instanceof DomainException) {
        final Map<String, Object> variables = CamundaErrorUtil.buildBpmnError(
          ex.getMessage(),
          State.CREDIT_AMOUNT.getTitle());
